...
class FindCode:
    ...
    def bring_table(self):
        ...
        fin_df = pd.read_csv("./data/finance.csv")
    ...
    def check_fin_conditions(self, searching, cond):
        ...
        for k, v in cond_idx.items():
            cond_arr = cond[str(k)]
            if cond_arr[0] is None and cond_arr[1] is None:
                continue
            else:
    ...
    def get_correct_date(self, codes, searching, i, ref_date):
        searching = searching[
            (searching[:, 3] < ref_date[i][1].date()) & (searching[:, 3] >= ref_date[i][0].date())]  # set_date
...
class Calculate:
    ...
    def calculate_profit(self, code_list):
        ...
        chart = sum(chart, [])
        dt = datetime(acc_profit[0][1][0].year, acc_profit[0][1][0].month, 31, tzinfo=timezone.utc)
        return_dict = dict(cumulative_return=acc_profit[-1][0][-1],
                           annual_average_return=annual_average_return[-1],
                           winning_percentage=winning_percentage,
                           max_loss_rate=max_loss_rate,
                           holdings_count=holdings_count[0],
                           chart=dict(start_date=dt, profit_rate_data=chart))
        return return_dict
